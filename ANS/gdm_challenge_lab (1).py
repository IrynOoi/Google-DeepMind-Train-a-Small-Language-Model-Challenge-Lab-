...
Original file is located at
    https://colab.research.google.com/drive/1wuX_C_Neky6lt60GBaV4rAfZweaglkUl
...
def generate_text_from_ngram_model(
    ...
    # Determine n from the model (context length = n - 1)
    # We can infer this from any key in the model
    context_length = len(next(iter(ngram_model.keys())))

    for _ in range(n_tokens):
        # Get the current context (last n-1 tokens)
        context_tokens = generated_tokens[-context_length:]
        context = tokenizer.join_text(context_tokens)

        # If context not in model, stop generation
...
def segment_encoded_sequence(
    ...
    if n_overlap >= max_length:
        raise ValueError("n_overlap must be smaller than max_length")

    i = 0
    while i < len(sequence):
        # Take a segment of max_length starting at i
        segment = sequence[i:i + max_length]
        subsequences.append(segment)

        if len(segment) < max_length:
            # Reached the last segment, stop
            break

        # Move forward by max_length - n_overlap
        i += max_length - n_overlap
...
def create_training_sequences(
  ...
  # -------------------------
    # Encode and segment dataset
    # -------------------------
    for text in dataset:
        # 1. Encode text into token IDs
        token_ids = tokenizer.encode(text)

        # 2. Segment the token_ids into overlapping subsequences
        segments = segment_encoded_sequence(token_ids, segmentation_length, n_overlap)

        # 3. Add all segments to encoded_tokens
