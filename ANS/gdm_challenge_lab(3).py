...
Original file is located at
    https://colab.research.google.com/embedded/projects/qwiklabs-gcp-04-ba606d3d372f/locations/us-west1/repositories/99ae0b34-707b-4703-bf88-c88c831dcc7f
...
class SimpleArabicCharacterTokenizer:
    ...
    def character_tokenize(self, text: str) -> list[str]:
        ...
        tokens = list(text)  # Add your code here.
    ...
    def join_text(self, tokens: list[str]) -> str:
        ...
        text = "".join(tokens) # Add your code here.
...
def generate_text_from_ngram_model(
  ...
  # Determine the context size (n-1) from the model keys
    # We grab an arbitrary key from the dictionary to check its length
    if not ngram_model:
        return start_prompt

    context_size = len(list(ngram_model.keys())[0])

    for _ in range(n_tokens):
        # 1. Update context: Get the last (n-1) tokens
        # We must join them into a string because the model keys are strings
        current_context = "".join(generated_tokens[-context_size:])

        # If the context doesn't exist in the model, stop generating
        if current_context not in ngram_model:
            break

        # 2. Get probabilities
        next_token_probs = ngram_model[current_context]
        possible_tokens = list(next_token_probs.keys())
        probabilities = list(next_token_probs.values())

        # 3. Support random or greedy sampling
        if sampling_mode == "greedy":
            # Use the provided argmax helper function
            next_token_index = argmax(probabilities)
            next_token = possible_tokens[next_token_index]
        else:
            # Use random.choices for weighted random sampling
...
def segment_encoded_sequence(
    ...
    # 1. Repeatedly take a segment
    # Use a step size to create overlaps
    for i in range(0, len(sequence), step):
        # Slice the sequence from i to i + max_length
...
def create_training_sequences(
    ...
    for text in dataset:
        ...
        # 3. Segment the sequence
        # We use segmentation_length (context + 1) to handle the target shift
